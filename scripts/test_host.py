...
v=json.load(open('protocol/golden_vectors.json'))['vectors'];(build/'golden.hex').write_text('\n'.join(x['frame_hex'] for x in v)+'\n')
...
for name,index in [('approved',2),('paused',2),('long',15),('unknown',2),('no-session',5),('reconnecting',2),('error',2)]:
...
print('PASS: production renderer generated all seven required 128x64 simulated states')
