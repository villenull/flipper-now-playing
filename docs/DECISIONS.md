...
## v0.3 Quick Settings (2026-09-12)

User authorized an additive Quick Settings tile and a downloadable GitHub APK update. Keep setup/launcher/settings. Standard (non-active) tile listening refreshes actual process state every time the shade opens; state observers update visible tiles without polling or keeping TileService alive. Enabled means the user-started bridge is armed, even while waiting or Bluetooth is off. Process-local state resets to Off after death. Removing the tile does not stop a listening session.

The tile starts a small visible TileStartActivity using startActivityAndCollapse (PendingIntent on API34+, legacy Intent only below34). It starts the connectedDevice foreground service only after resume and window focus and remains visible until foreground entry. This retains the original visible-start constraint and avoids assuming a tile binding grants unrestricted background-start/audio rights. Starting requires unlock; stopping is allowed while locked. Long-press uses ACTION_QS_TILE_PREFERENCES. No new uses-permission entries or dependencies.

Android references reviewed: https://developer.android.com/develop/ui/views/quicksettings-tiles ; https://developer.android.com/about/versions/14/behavior-changes-14 ; https://developer.android.com/about/versions/17/changes/bg-audio . Source guidance is not Android17 physical audio evidence. Shared Bluetooth/media access checks prevent the tile from skipping setup. Original certificate SHA256 84af0fed14f74dc3d177870628d375ce0577364150036348c72ee9e736c25251 verified locally; optional NP_DEBUG_KEYSTORE config supports explicit release signing with this existing key. Release v0.3 uses Android code3/name0.3.

AGP8.10 lint flags the legacy startActivityAndCollapse(Intent) call even behind the API34 branch. Keep a narrowly annotated legacy helper with an explicit runtime API<34 check; newer systems always use the PendingIntent overload. This is a compatibility-only suppression, not a skipped API34 fix. Lint's unrelated stopService SAM false positive is avoided by naming the Intent variable.
