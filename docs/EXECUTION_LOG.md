...
## v0.3 Quick Settings — implementation and validation in progress (2026-09-12)
- Read current implementation and Android primary documentation. User authorized implementation plus GitHub APK publication.
- Implemented permission-protected toggle TileService, standard visible listening with bounded observer lifecycle, a visible automatic foreground-service start activity, missing-setup routing, long-press preferences and Add to Quick Settings prompt/manual fallback. Shared setup checks and separate enabled/connection state avoid stale Off/On assumptions. Five policy tests added.
- Recreated pinned .cache using previously recorded SDK authorization: bootstrap exit0. doctor exit0. JDK17.0.16+8, Gradle8.11.1, SDK36/build-tools35, firmware1.4.3/API87.1.
- Existing debug keystore certificate matches original84af0fed...25251; no signing identity change. No secret contents printed or added to source.
- Sandboxed first pipeline passed30 reference/C sanitizer/renderer checks then failed Gradle distribution network access. Restarted with authorized network and NP_DEBUG_KEYSTORE. No test suppressed.
- Read-only authorized adb devices exit0: no devices. Tile/system UI/Android17 behavior pending. Current milestone: APK build/lint/unit suite running; next inspect result, fix actual failures, complete full pipeline, package and publish v0.3, verify public downloads.

- First authorized Android run compiled code and all five tile policy tests passed, but lint failed on legacy startActivityAndCollapse despite the API guard, plus the known stopService SAM warning. Isolated the legacy API call with a runtime API<34 check and narrow documented lint suppression; named the stop Intent. Added revoked-access/READY coverage and ensured failed start returns to setup immediately. Final full pipeline restarted with stable source/scripts.

- Excluded Kotlin's generated .kotlin/sessions cache from Git, source digest and source archives; it appeared during the fresh toolchain build. This packaging-only change was made before the final validation-stamp stage.
